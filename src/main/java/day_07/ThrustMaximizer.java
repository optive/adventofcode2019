...
public class ThrustMaximizer {
    ...
    public int calculateOutputSignal(final List<Integer> phaseSequence, final String program) {
        int input = 0;
        for (final int phase : phaseSequence) {
            final Amplifier amplifier = new Amplifier(program, new LinkedList<>(Arrays.asList(phase, input)));
            input = amplifier.getOutput();
        }
        return input;
    }

    public int calculateFeedbackSignal(final List<Integer> phaseSequence, final String program) {
        Integer input = 0;
        ...
        for (int i = 0; i < phaseSequence.size(); i++) {
            final Queue<Integer> inputQueue = new LinkedList<>(Arrays.asList(phaseSequence.get(i), input));
    ...
    public static void main(String[] args) throws IOException {
        ...
        final List<Integer> secondOutputs = new ArrayList<>();
